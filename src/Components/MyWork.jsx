  ...
  return (
    <div id="work" className="flex flex-col items-center justify-center gap-20 mx-32 my-28 max-md:items-start max-md:mx-10">
        <div className="relative">
            <h1 className="text-6xl font-semibold text-center max-md:text-5xl">My Latest Work</h1>
            <img src={theme_pattern} alt="theme" className="absolute h-14 top-3 left-[17rem] -z-10 max-md:h-10 max-md:left-64"/>
        </div>

        <div className="grid grid-cols-3 gap-10 max-md:grid-cols-2">
            {mywork_data.map((work, index) =>{
                return <img src={work.w_img} className="h-60 w-96 rounded-2xl box-border cursor-pointer transform transition-transform duration-300 hover:scale-110 hover:border-4 border-fuchsia-500 hover:rounded-xl" alt="work_img" key={index}/>
            })}
        </div>
